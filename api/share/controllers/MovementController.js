...
exports.create = async (req, res) => {
  ...
  if (type === 'Ajuste de estoque') {
    ...
    //(destination)
    let localDestination = await Local.findOne({ where: { name: destination } })
    //console.log(localDestination)
    let filter = { localId: localDestination.id, componentId: targetItem.componentId }
    const item = await Item.findOne({ where: filter })

    if (item) {
      const updatedFieldsItem = {
        quantity: parseFloat(item.quantity) + parseFloat(quantity),
        updatedAt: moment.tz('America/Sao_Paulo').format()
      };
      await item.update(updatedFieldsItem)
    } else {
      newItem = new Item({
        quantity: quantity,
        minimum: 0,
        localId: localDestination.id,
        adress: 'NOVO',
        componentId: targetItem.componentId,
      })
      await newItem.save()

    }
    //CREATE MOVEMENT IN DESTINY
    const movementDestination = new Movement({
      type: 'Entrada de material',
      quantity: quantity,
      destination: 'NOVO',
      itemId: newItem.id,
      userId: userId,
    ...
    await movementDestination.save();
    //VERIFICAR A NECESSIDADE DE CONFIRMAÇÃO DA ENTRADA FISICA DO ITEM
    //const users = User.findAll({where: {admin: true}});
    //MailService.sendTransfer(users);
  }
