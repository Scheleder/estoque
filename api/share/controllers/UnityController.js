...
exports.create = async(req, res)=>{
  const {name, abrev, decimal, userId} =  req.body
  if(!name){
  ...
  if(!decimal){
    return res.status(202).json({ msg:"Número de casas decimais é obrigatório!"})
  }
  //CHECK LOCAL
...
exports.update = async(req, res)=>{
  ...
  const updatedFields = {
    ...
    abrev: abrev || unity.abrev,
    decimal: decimal || unity.decimal,
