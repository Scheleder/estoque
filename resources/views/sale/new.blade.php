<x-app-layout>
    <div class="mt-8 sm:mt-1 flex justify-center p-6 gap-4">
        <article class="text-center rounded-lg bg-gray-300 p-4 w-screen shadow-lg">
            <span class="text-gray-600 font-semibold mr-4"><a data-modal-target="edit-client-modal"
                    data-modal-toggle="edit-client-modal" title="{{ __('Edit Data') }}">{{ $client->name }}</a></span>
            <span class="cursor-pointer font-medium" onclick="openWhats({{ $client->whatsapp }})" title="Abrir WhastApp"><i
                    id="whats" class="fab fa-whatsapp text-green-600 font-extrabold">
                    <script>
                        formatPhone({{ $client->whatsapp }}, "whats")
                    </script>
                </i></span><br>
            <span class="text-blue-900 font-thin"><a data-modal-target="edit-client-modal"
                    data-modal-toggle="edit-client-modal" title="{{ __('Edit Data') }}">{{ $client->adress }}</a></span>
        </article>
    </div>
    ...
    <article class="mx-4 mt-12 sm:mt-1 p-2 rounded-xl bg-claro dark:bg-gray-800 shadow-lg hover:shadow-xl">
        ...
        <div class="px-4 py-6 lg:px-8">
            <h2 class="mb-6 text-xl uppercase text-cor-70 dark:text-gray-200 font-bold"><i
                    class="fas fa-cart-plus fa-lg">&nbsp;</i>&nbsp;{{ __('New Sale') }}</h2>
            <form class="space-y-6" action="/sale/items" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
                ...
                <div class="items">
                    ...
                    <label for="products" class="block mb-1 text-sm font-bold text-gray-600 dark:text-white"><i
                            class="fas fa-shopping-cart">&nbsp;</i>&nbsp;{{ __('Products') }}</label>
                    <div class="row" id="search-div">
                        <select id="js-example-basic-multiple" value="${product_item}" multiple="multiple"
                            class="w-full search-products dark:bg-gray-200">
                    ...
                    <div id="barcode-div" style="display:none"><input
                            class="w-full h-20 mb-2 border-pink-300 rounded-lg focus:ring-pink-500" id="barcode-input"
                            type="text" placeholder="Utilize o leitor de códigos de barras"></div>

                    <div class="mt-2 mb-8 relative overflow-x-auto shadow-md rounded-lg">
                        <table class=" w-full text-sm text-left text-gray-500">
                            <thead
                                class="text-xs text-claro uppercase border-b-2 border-claro bg-gradient-to-r from-cor-70 to-cor-50">
                                <tr>
                    ...
                    <div class="row md:flex">
                        <div class="col w-full md:w-1/3">
                            <label for="discount" class="block mb-1 text-sm font-bold text-cor-80 dark:text-gray-200"><i
                                    class="fas fa-tags">&nbsp;</i>&nbsp;{{ __('Discount') }}</label>
                            <input type="text" name="discount" id="discount" onfocus="this.value='';"
                                onblur="carregarItems()"
                        ...
                        <div class="col w-full md:w-1/3 md:pl-6">
                            <label for="delivery" class="block mb-1 text-sm font-bold text-cor-80 dark:text-gray-200"><i
                                    class="fas fa-truck">&nbsp;</i>&nbsp;{{ __('Delivery') }}</label>
                            <input type="text" name="delivery" id="delivery" onfocus="this.value='';"
                                onblur="carregarItems()"
                                class="mb-3 bg-bege border border-cinza-claro text-cor-70 text-sm rounded-lg focus:ring-cor-70 focus:border-cor-70 block w-full p-2.5"
                                placeholder="R$ 0,00">
                        </div>
                        <div class="col w-full md:w-1/3 md:pl-6">
                            <label for="value" class="block mb-1 text-sm font-bold text-cor-80 dark:text-gray-200"><i
                                    class="fas fa-money-bill-wave">&nbsp;</i>&nbsp;{{ __('Total Value') }}</label>
                            <input type="text" name="value" id="value" readonly
                                class="mb-3 bg-bege border border-cinza-claro text-cor-70 text-sm rounded-lg focus:ring-cor-70 focus:border-cor-70 block w-full p-2.5"
                                placeholder="R$ 0,00">
                        </div>
                    </div>
                    <label for="description" class="block mb-1 text-sm font-bold text-cor-80 dark:text-gray-200"><i
                            class="fas fa-info-circle">&nbsp;</i>&nbsp;{{ __('Observations') }}</label>
                    <textarea type="text" name="description" id="description" rows="2"
                        ...
                        placeholder="Observações... (Não obrigatório!)"></textarea>
                </div>
                ...
                <button type="submit" title="{{ __('Add Sale') }}"
    ...
    <script type="text/javascript">
        ...
        $('#js-example-basic-multiple').on('select2:select', function(e) {
            console.log('Adicionou: ' + e.params.data.text);
        ...
        $('#js-example-basic-multiple').on('select2:unselect', function(e) {
            console.log('Removeu: ' + e.params.data.text);
            if (items.length > 0) {
                index = items.indexOf(items.filter(a => a.id === e.params.data.id)[0]);
        ...
        var carregarItems = function() {
            ...
            items.forEach((el) => {
                ...
                let total_item = (el.qtty * el.value).toFixed(2);
                soma_total += (el.qtty * el.value);
                if (el) {
                    //let dado_container = `<tr class="even:bg-pink-100 odd:bg-pink-50 hover:bg-pink-200 h-6"><td class='text-center'><i class='fas fa-edit hover:text-blue-700 ml-2' title='{{ __('Update Amount') }}' onclick='alteraQuantidade(${index}, ${el.qtty})'></i></td><td class='px-3 py-1 text-center'>${ el.qtty }</td><td class='px-3 py-1'>${ el.description }</td><td class='px-3 py-1 text-right max-sm:hidden'>R$ ${ el.value}</td><td class='px-3 py-1 text-right'>R$ ${ total_item }</td><td class='px-3 py-1 text-center'><i class='fas fa-trash hover:text-red-700 ml-2' title='{{ __('Delete') }}' onclick='removeItem(${el.id})'></i></td></tr>`;
                    let dado_container =
                        `<tr class="even:bg-cinza-claro  odd:bg-bege hover:bg-cor-50 text-cor-80 h-6"><td class='text-center'><i class='fas fa-edit hover:text-warning ml-2 cursor-pointer' title='{{ __('Update Amount') }}' onclick='alteraQuantidade(${index}, ${el.qtty}, ${el.amount})'></i></td><td class='px-2 py-1 text-center'>${ el.qtty }</td><td class='px-2 py-1'>${ el.description }</td><td class='px-2 py-1 text-right max-sm:hidden'>R$ ${ el.value}</td><td class='px-2 py-1 text-right'>R$ ${ total_item }</td><td class='px-2 py-1 text-center'><i class='fas fa-trash hover:text-danger ml-2 cursor-pointer' title='{{ __('Remove Item') }}' onclick='removeItem(${el.id})'></i></td></tr>`;
